...
export const ChatWidget: React.FC<ChatWidgetProps> = ({
  ...
  async function ensureConversation() {
    ...
    const id = await client.startConversation().catch(err => { onError?.(err); throw err })
  ...
  return (
    <>
      {!open && (
        <button onClick={handleOpen} style={{ background: accentColor }} className={`zcw-launch-btn ${className}`}>Chat</button>
      )}
